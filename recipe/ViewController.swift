...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        TextViews.text = ""
        let jsonURLString = "http://www.recipepuppy.com/api/?i=onions,garlic&q=omelet&p=1"
        ...
        URLSession.shared.dataTask(with: urlObj) {(data, responds, err) in
            ...
            do {
                ...
                for countrys in countries.results {
                    self.TextViews.text.append("Food name : \(countrys.title!) \n")
                    self.TextViews.text.append("Material : \(countrys.ingredients) \n")
                    self.TextViews.text.append("================================ \n")
                }
